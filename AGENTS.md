...
The macOS app lives under `hackfm/hackfm`. Source is organized by layer: `App/` (entry point), `Views/` (SwiftUI UI), `ViewModels/` (state + orchestration), `Services/` (audio processing, HackRF integration, streaming), and `Models/` (shared types). App support files live in `Supporting/` (entitlements, bridging header). The Xcode project is `hackfm/hackfm.xcodeproj`. Product notes are in `specs/prd.md`. Build artifacts land in `hackfm/build/` and are git-ignored.
...
- Open in Xcode: `open hackfm/hackfm.xcodeproj`
- Build from CLI: `xcodebuild -project hackfm/hackfm.xcodeproj -scheme HackFM -configuration Debug build`
...
There is no automated test target in this repository today. Validate changes manually by building, launching the app, loading an audio file, selecting a frequency, and starting/stopping transmission. If you add tests, create an `XCTest` target and place files under a `HackFMTests/` directory with names like `AudioProcessorTests.swift`.
